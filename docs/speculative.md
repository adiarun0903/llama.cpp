...
Note: TurboQ v2 uses `--memory-codec turboq`. The speculative examples are no longer categorically blocked on TurboQ; supported causal models can use both draft-model and draftless speculative paths directly on TurboQ memory. MLA and SWA cache variants still remain outside the TurboQ v2 target set.
